...
class Torrent
{
  ...
  {
    return ($start = self::file_get_contents($file, $timeout, 0, 11)) && $start === 'd8:announce' || $start === 'd10:created' || $start === 'd13:creatio' || $start === 'd13:announc' || $start === 'd12:_info_l' || substr($start, 0, 10) === 'd7:comment' // @see https://github.com/adriengibrat/torrent-rw/issues/32
      || substr($start, 0, 7) === 'd4:info' || substr($start, 0, 3) === 'd9:'; // @see https://github.com/adriengibrat/torrent-rw/pull/17
  }
  ...
  static public function file_get_contents($file, $timeout = self::timeout, $offset = null, $length = null)
  {
    if (is_file($file) || ini_get('allow_url_fopen')) {
      $context = !is_file($file) && $timeout ? stream_context_create(array(
        'http' => array(
          'timeout' => $timeout
        )
      )) : null;
