...
/*        if (preg_match("/($hostname)/i", $data['host'])) {
    ...
    if (count($_ERROR) == 0) {
        ...
        $short_url1 = SITE_URL."/".$code;
        $short_url = "http://shg.li/".$code;
        $_GET['url']   = "";
        $_GET['alias'] = "";
        $info = "<p class=info>结果：<strong><a href=\"".htmlentities($short_url1) ."\" title=\"".htmlentities($short_url1)."\" target=\"_blank\">".htmlentities($short_url1)."</a></strong> </p>";
        $info .= "<p class=info>结果：<strong><a href=\"".htmlentities($short_url) ."\" title=\"".htmlentities($short_url)."\" target=\"_blank\">".htmlentities($short_url)."</a></strong> </p>";
